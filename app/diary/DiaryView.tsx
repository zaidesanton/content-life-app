...
function TodayEditor({ todayStr, initial }: { todayStr: string; initial: DiaryEntry | null }) {
  ...
  return (
    <div className="space-y-5">
      {/* Score */}
      <div>
        <p className="text-[10px] font-semibold text-[#999] uppercase tracking-widest mb-2.5">
          How was your day?
        </p>
      ...
      <div>
        ...
        <input
          ...
          className="w-full bg-[#111] border border-[#252525] rounded-lg px-3 py-2.5 text-[13px] text-[#ccc] placeholder:text-[#666] focus:outline-none focus:border-[#444]"
        />
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          rows={5}
          className="w-full bg-[#111] border border-[#252525] rounded-lg px-3 py-2.5 text-[13px] text-[#ccc] placeholder:text-[#666] focus:outline-none focus:border-[#444] resize-none leading-relaxed"
        />
      </div>
    </div>
  )
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3">
        <span className="text-[12px] text-[#888] shrink-0 w-[88px]">{fmt(entry.date)}</span>
        {entry.score ? (
          <span className={`text-[12px] font-bold tabular-nums shrink-0 ${SCORE_COLOR[entry.score]}`}>
            {entry.score}/10
          </span>
        ) : (
          <span className="text-[12px] text-[#777] shrink-0">—</span>
        )}
        {preview && (
          <span className="text-[12px] text-[#888] truncate flex-1">{preview}</span>
        )}
        <span className={`text-[10px] text-[#777] shrink-0 transition-transform duration-200 ${expanded ? 'rotate-180' : ''}`}>
          ▾
        </span>
      </div>
      ...
      {expanded && (
        <div className="mt-3 space-y-3 pl-[100px]">
          {entry.good_thing && (
            <div>
              <p className="text-[9px] font-semibold text-[#888] uppercase tracking-widest mb-1">Good thing</p>
              <p className="text-[13px] text-[#888]">{entry.good_thing}</p>
            </div>
          )}
          {entry.thoughts && (
            <div>
              <p className="text-[9px] font-semibold text-[#888] uppercase tracking-widest mb-1">Thoughts</p>
              <p className="text-[13px] text-[#777] whitespace-pre-wrap leading-relaxed">{entry.thoughts}</p>
            </div>
          )}
          {!entry.good_thing && !entry.thoughts && (
            <p className="text-[12px] text-[#777] italic">No notes recorded.</p>
          )}
        </div>
      )}
    </div>
...
export default function DiaryView({
  ...
  return (
    <div className="px-6 md:px-8 pt-8 pb-10 max-w-xl">
      ...
      <p className="text-[12px] text-[#888] mb-7">
